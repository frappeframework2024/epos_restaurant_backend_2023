<template>
    <div v-if="countPendingSaleListResource.data > 0 && !countPendingSaleListResource.loading" class="mx-4">
        <v-alert
            ...
            variant="tonal">
            <template #title>
                <span style="font-family: Khmer OS Siemreap;">{{ $t('Pending Order') }}</span>
            </template>
            <template #text>
                <div :class="mobile ? '' : 'flex justify-between items-center'">
                    <div> <span style="font-family: Khmer OS Siemreap;" class="font-bold underline cursor-pointer" @click="onViewPendingOrder">{{$t(`msg.There are pending orders`,[ countPendingSaleListResource.data ])}}</span></div>
                    <div class="p-2"><v-btn style="font-family: Khmer OS Siemreap;" color="primary" @click="onViewPendingOrder">{{ $t('View Pending Order') }}</v-btn></div>
                </div>
            </template>
        </v-alert>
    </div>
