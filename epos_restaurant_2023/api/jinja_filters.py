...
def format_currency(value):
    ...
    precision = frappe.get_cached_value(
        "Currency", currency, "custom_currency_precision"
    )
...
def get_exchange_rate(date):
    from_currency = frappe.get_cached_value(
        "ePOS Settings", None, "exchange_rate_main_currency"
    )
    ...
    currecy_data = frappe.db.sql(
        "select exchange_rate_input as exchange_rate_input from `tabCurrency Exchange` where posting_date<=%(posting_date)s and from_currency = %(from_currency)s and to_currency = %(to_currency)s order by creation desc limit 1",
        {
            "posting_date": date,
...
def get_combo_product_by_kitchen_group(value=None):
    ...
    for p in value:
        p["kitchen_group"] = (
            frappe.get_cached_value("Product", p.get("product_code"), "kitchen_group")
            or "Not Set"
        )

    # get sorted kitchen group
    sql = "select name as kitchen_group from `tabKitchen Group` where name in %(names)s order by sort_order"
    kitchen_group = frappe.db.sql(
        sql, {"names": [d.get("kitchen_group") for d in value]}, as_dict=1
    )

    for k in kitchen_group:
        k["products"] = [
            d for d in value if d.get("kitchen_group") == k.get("kitchen_group")
        ]
