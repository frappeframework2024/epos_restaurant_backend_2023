...
class ExpensePayments(Document):
	def validate(self):
...
def update_expense(self):
	data = frappe.db.sql("select  ifnull(sum(payment_amount),0)  as total_paid from `tabExpense Payments` where docstatus=1 and expense='{}'".format(self.expense))
