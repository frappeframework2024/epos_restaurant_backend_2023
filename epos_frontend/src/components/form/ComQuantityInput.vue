<template>
    <div v-if="!saleProduct.is_timer_product">
        ...
        <v-chip class="ml-1 mb-1" size="small" color="error"
            v-if="saleProduct.deleted_quantity > 0 && gv.device_setting.show_deleted_sale_product_in_sale_screen == 1"
            variant="outlined">{{ `${$t('QTY Deleted')}: ${saleProduct.deleted_quantity}` }} </v-chip>
        ...
        <v-btn
            ...
            color="error" size="x-small" variant="tonal" icon="mdi-arrow-down" @click="onUpdateQuantity(-1)"
        ...
        <v-btn
            v-if="(saleProduct.sale_product_status == 'New' && saleProduct.append_quantity == 1 && saleProduct.is_require_employee == 0) || sale.setting.pos_setting.allow_change_quantity_after_submit == 1"
            color="success" size="x-small" variant="tonal" icon="mdi-arrow-up" @click="onUpdateQuantity(1)"></v-btn>
    </div>
    <div v-else>
        ...
        <div v-if="saleProduct.time_in && !saleProduct.time_out_price">
            <span>{{ stopwatch.hours.toString().padStart(2, '0')
                }}h</span>:<span>{{ stopwatch.minutes.toString().padStart(2,
        '0') }}mn</span>:<span>{{ stopwatch.seconds }}s</span>
        </div>
