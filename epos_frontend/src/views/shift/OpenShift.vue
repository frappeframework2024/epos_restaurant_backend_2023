<template>
    <PageLayout :title="$t('Open Shift')" icon="mdi-clock" class="p-3">
        ...
        <v-row>
            <v-col cols="12">
                <ComInput :title="$t('Enter Note')" :label="$t('Open Note')" v-model="opened_note" type="textarea" keyboard>
                </ComInput>
            </v-col>
        </v-row>
        <div style="font-family: Khmer OS Siemreap;" class="flex items-center justify-between mt-8 mb-3">
...
async function onOpenShift() {
    db.getDocList("Cashier Shift", {
        ...
        let message = shift_type.value + " is already opened or closed. Are you want to continue open?"
        if (res.length > 0) {
            if (await confirm({ title: $t("Warning"), text: $t(message) })) {
