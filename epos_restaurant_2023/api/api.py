...
def search_image_from_google(keyword):
    ...
    url = "https://google-image.inccloudserver.com/google/image/search"
...
def check_valid_sale_product_as_coupon(**args):
    ...
    sql = """    
        ...
        b AS (
            ...
            WHERE docstatus = 1 and coupon_code = %(coupon_code)s
        )
        SELECT 
            *, 
    ...
    response["data"] = {
        ...
        "coupon_amount": float(result.get("coupon_amount")),
        "coupon_used_amount": float(result.get("coupon_used_amount")) + float(total_coupon_payment),
        "remaining_amount": float(result.get("remaining_amount")) - float(total_coupon_payment),
    }
    return response
...
def get_system_settings(pos_profile="", device_name=''):
    ...
    data={
        ...
        "allow_change_date_on_start_working_day":pos_config.allow_change_date_on_start_working_day,
        "print_new_deleted_sale_product":pos_config.print_new_deleted_sale_product,
        "allow_print_bill_on_sale_deleted":pos_config.allow_print_bill_on_sale_deleted,
        "print_sale_product_merged_table":pos_config.print_sale_product_merged_table,
        "print_sale_product_change_table":pos_config.print_sale_product_change_table,
