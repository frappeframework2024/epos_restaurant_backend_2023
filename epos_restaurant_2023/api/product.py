...
@frappe.whitelist()
def get_currenct_cost(product_code="",stock_location="",unit="",price_rule = ""):
    ...
    if product.default_account:
        stock_account = product.default_account[0].default_stock_account
    ...
    product_price_lists = frappe.db.sql("select * from `tabProduct Price` where parent = '{0}' and unit = '{1}' and price_rule = '{2}'".format(product_code,unit,price_rule))
    price = product.price
    if product_price_lists:
        price = product_price_lists[0].price
    if stock_location_products:
        if stock_location_products[0].count>0:
            ...
            if doc or len(doc)>0:
                ...
                doc[0]["price"] = price
                doc[0]["stock_account"] = stock_account
                return doc[0]
            else:
                 return {"cost":product.cost / uom_conversion ,"quantity":0, "price":0,"stock_account":stock_account}
        else:
...
def get_products(category ='All Product Categories',product_code=None,keyword=None , limit = 20, page=1, order_by='product_code',order_by_type='asc', include_product_category=0,price_rule="Normal"):
    ...
    operator = ""
    if keyword:
        filter["keyword"] = '%{}%'.format((keyword or ""))
        operator = "like"
    else:
    ...
    if int(include_product_category) ==1:
        ...
        return {"products":data,"unit":product_price_unit}
