<template>
    <div class="h-full">
        ...
        <div class="overflow-y-auto p-1">
            ...
            <v-select  style='font-family: "Khmer OS Siemreap";' v-if="reportOption.show_cashier_shift" :label="$t('Cashier Shift')" :items="filterResource.data?.cashier_shifts"
            ...
            <v-select  style='font-family: "Khmer OS Siemreap";' v-if="reportOption.show_table_group" :label="$t('Table Group')" :items="filterResource.data?.table_groups"
                variant="solo" v-model="filter.table_group" item-title="title" item-value="name"></v-select>

            <v-select  style='font-family: "Khmer OS Siemreap";' v-if="reportOption.show_sale_type" :label="$t('Sale Type')" :items="filterResource.data?.sale_types" variant="solo"
                v-model="filter.sale_type" item-title="title" item-value="name"></v-select>

            <v-select  style='font-family: "Khmer OS Siemreap";' v-if="reportOption.show_payment_type" :label="$t('Payment Type')" :items="paymentTypes" variant="solo"
                v-model="filter.payment_type" item-title="payment_method" item-value="payment_method"></v-select>
          
            <div class="flex justify-between -m-1">
                <div class="m-1 grow">
                    <v-select  style='font-family: "Khmer OS Siemreap";' v-if="reportOption.show_order_by" :label="$t('Sort Order by')" :items="orderByOptions" variant="solo"
                    v-model="filter.order_by" item-title="title" item-value="name"></v-select>
                </div>
                <div class="m-1 flex-none">
                    <v-select  style='font-family: "Khmer OS Siemreap";' v-if="reportOption.show_order_by" :label="$t('Sort Order Type')" :items="orderByType" variant="solo"
        ...
        <div class="flex items-end pt-1">
            <v-btn  prepend-icon="mdi-magnify" block size="large" @click="onSearch" color="searchbtn">
                <span style='font-family: "Khmer OS Siemreap";'>{{$t('Search')}}</span>
