...
frappe.views.QueryReport = class QueryReport extends frappe.views.BaseList {
		...
		return new Promise((resolve) => {
			...
			.then((r) => {
				...
				if (data.result && data.result.length) {
					...
					//we use this to render summary frappe table under main table
					const summary_data_for_table = []
					if(document.getElementById("summary_data")){
						document.querySelector("#summary_data").remove()
					}

					if(applied_filters.chart_type == "pie"){
						frappe.show_alert(__('Use one Series Chart to see clear data in Pie Chart'));
					}
					
					if(applied_filters.summary_filter){
						const group_record=[...(new Set(data.result.filter(x=>x[applied_filters.summary_filter]).map(r=>r[applied_filters.summary_filter])))];
						 //get group column info
						const group_column =  (this.filters.filter(f=>f.df.fieldname=="summary_filter")[0])
												.get_data().filter(r=>r.value == applied_filters.summary_filter)[0]
						
						group_column["name"] = group_column.description	
						summary_columns.push(group_column)

			 
						const data_coumns = this.filters.filter(f=>f.df.fieldname=="summary_fields")[0].get_data()
						
						if((applied_filters.summary_fields || []).length==0){
							
							applied_filters.summary_fields = data_coumns.map(r=>r.value)
						}

						applied_filters.summary_fields.forEach(f=>{
						...
						if(applied_filters.summary_fields){
							group_record.forEach(g=>{
								let summary_table_record = []
								//generate column 
								let record = {}
								record[group_column.value] = g
								//we use this to render summary frappe table under main table
								summary_table_record.push(frappe.format(g,{"fieldtype":group_column.fieldtype}) || g)

								
								applied_filters.summary_fields.forEach(f=>{
									record[f] = data.result.filter(d=>d.is_data==1 && d[applied_filters.summary_filter]==g ).reduce((n, d) => n + (d[f] || 0), 0)
									//we use this to render summary frappe table under main table
									summary_table_record.push(frappe.format( record[f] ,{"fieldtype":summary_columns.find(r=>r.value==f).fieldtype}) || record[f] )
								})
								summary_data.push(record)
								//we use this to render summary frappe table under main table
								summary_data_for_table.push(summary_table_record)

							})
							//total row
							let total_record = {}
							//we use this to render summary frappe table under main table
							let total_summary_table_record = ["Total"]
							total_record[group_column.value] = "Total"
							total_record.is_total_row =1
							applied_filters.summary_fields.forEach(f=>{
								total_record[f] = data.result.filter(d=>d.is_data==1 ).reduce((n, d) => n + (d[f] || 0), 0)
								//we use this to render summary frappe table under main table
								total_summary_table_record.push(frappe.format(total_record[f]  ,{"fieldtype":summary_columns.find(r=>r.value==f).fieldtype}) || total_record[f]  )
							})
							summary_data.push(total_record)

							//we use this to render summary frappe table under main table
							summary_data_for_table.push(total_summary_table_record)
							 

							window.summary_data = summary_data
							window.summary_columns = summary_columns
						...
						window.summary_data = null
						window.summary_columns = null
					}
					//end show summary
	...
	pdf_report(print_settings) {
		...
		const reportSummaryString = this.raw_data.report_summary? document.querySelector(".report-summary").outerHTML:""
		let stringSummary = ""
		let summaryColumn = landscape?8:7;
		const summaryFontSize = landscape?14:11;
		if (reportSummaryString){
			const domReportSummary = parser.parseFromString(reportSummaryString, "text/html");
			const items = domReportSummary.querySelectorAll(".summary-item")
			const max_row = Math.ceil(items.length / 8)
			stringSummary = "<table style='width:100%'>"
			for (let i = 0; i <max_row; i++) {
				stringSummary = stringSummary + "<tr>"
				for (let j = 0; j <summaryColumn; j++) {
					const index = j + summaryColumn * i
					if (index<items.length){
						stringSummary = stringSummary + `<td style='font-size:${summaryFontSize}px;text-align:center'>${items[index].querySelector(".summary-label").outerHTML}</td>`
					}else {
						break;
					}
				}
				...
				for (let j = 0; j <summaryColumn; j++) {
					const index = j + summaryColumn * i
					if (index<items.length){
						let valueDom = items[index].querySelector(".summary-value").getAttribute("class")
						let color=""
						if (valueDom){
							const arrValueDom = valueDom.split(" ")
							if (arrValueDom.length>1){
								color=arrValueDom[arrValueDom.length-1]
							}
						}
						stringSummary = stringSummary + `<td style='color:${color || 'black'};font-size:${summaryFontSize}px;text-align:center'>${items[index].querySelector(".summary-value").textContent}</td>`
					}else {
						break;
					}
				}
			...
			stringSummary = stringSummary  + "</table>"
	...
	get_filters_html_for_print() {
		...
		return Object.keys(applied_filters)
			.map((fieldname) => {
				...
				if((docfield.hide_in_filter || 0)==0){
