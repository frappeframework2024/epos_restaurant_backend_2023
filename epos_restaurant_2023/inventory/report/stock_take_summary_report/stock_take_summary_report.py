...
def get_columns(filters):
	...
	if (filters.row_group == "Stock Take Transaction" or filters.parent_row_group == "Stock Take Transaction") and filters.get("include_cancelled") == True:
...
def get_report_data(filters,parent_row_group=None,indent=0,group_filter=None):
	...
	sql = sql + """ {2}
		FROM `tabStock Take Products` AS a
