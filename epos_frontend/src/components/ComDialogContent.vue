            ...
            <div class="overflow-auto lg:overflow-hidden">
                ...
                <div class="flex gap-1 lg:gap-0 justify-between items-center w-max lg:w-full">
                    ...
                    <div class="flex gap-2">
                        ...
                        <v-btn style="font-family: Khmer OS Siemreap;" class="text-white" v-if="!hideButtonClose"  rounded="lg" color="red-lighten-1" @click="close()" :loading="loading">
                            {{ titleButtonClose ?? 'Cancel' }}
                        </v-btn>            
                        <v-btn style="font-family: Khmer OS Siemreap;" class="text-white" :disabled="disabledBtnOk" rounded="lg" color="green-lighten-1" v-if="!hideButtonOK" @click="onOK()" :loading="loading">
                            {{ titleButtonOK ?? 'Save' }}
                        </v-btn>
                    </div>
                </div>
