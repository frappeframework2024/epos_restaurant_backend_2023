<template>
    <PageLayout :title="$t('Working Day')" icon="mdi-calendar-clock">
        ...
        <template #action>
            <v-btn style="font-family: Khmer OS Siemreap;" v-if="setting?.pos_setting?.show_preview_report" prepend-icon="mdi-printer" @click="onOpenReport">{{ $t('Report') }}</v-btn>
        </template>
        <template #default>
            <div class="pa-4">
                ...
                <v-btn style="font-family: Khmer OS Siemreap;"
                ...
                <v-btn style="font-family: Khmer OS Siemreap;" @click="router.push({ name: 'Home' })" color="error" class="ml-4">{{ $t('Cancel') }}</v-btn>
            </div>
        </template>
    </PageLayout>
</template>
