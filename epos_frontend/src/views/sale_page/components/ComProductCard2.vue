<template>
  <div class="products-grid">
    <div v-for="prod in productsByCategory" :key="prod.name" class="product-card" @click="onProductClick(prod)">
      ...
      <div class="flex">
        <div class="product-image">
          ...
          <img v-else :src="getImage" :alt="prod.name" style="width: 100%;height: 100%; object-fit: cover;"
            @error="onImageError" loading="lazy" />
        </div>

        <div class="flex w-full" style="width: 60%;">
          <div class="product-info" style="width: 85%;">
            <div class="product-name" :style="{ fontSize: gv.itemMenuSetting.item_font_size + 'px' }"> {{
              getProductName(prod)
              }}<span style="color: red;">{{ getTotalQuantityOrder(prod) }}</span></div>

            <div class="product-price flex items-center"
              :style="{ fontSize: gv.itemMenuSetting.font_price_size + 'px' }">
          ...
          <div style="width: 15%;" class="flex items-end justify-end">
            <button v-ripple @click="onProductClick(prod)">
  ...
  <div v-if="previewProduct" class="image-overlay" @click="previewProduct = null">
    <div class="image-popup" @click.stop @wheel.prevent="onWheel" @touchstart.prevent="onTouchStart"
      @touchmove.prevent="onTouchMove" @touchend="onTouchEnd" @mousedown="onMouseDown" @mousemove="onMouseMove"
      @mouseup="onMouseUp" @mouseleave="onMouseUp" :style="{
        transform: `translate(${pan.x}px, ${pan.y}px) scale(${zoom})`,
        transformOrigin: 'center center',
        cursor: zoom > 1 ? (isDragging ? 'grabbing' : 'grab') : 'default'
      }">
      <button class="close-btn" @click.stop="previewProduct = null" @mousedown.stop @touchstart.stop>✕</button>
      <img v-if="previewProduct.photo" :src="previewProduct.photo" :alt="previewProduct.name" draggable="false"
