...
class BulkStockEntry(Document):
	...
	def before_submit(self):
		if self.transaction_type == "Purchase Order":
			...
			self.set("products",[row for row in self.products if flt(row.quantity) != 0])
			...
			self.set("products",[row for row in self.products if flt(row.quantity) != 0 or flt(row.cost) != flt(row.current_cost)])
		self.update_summary()
	...
	def get_product(self, product_code=None, options=None):
		...
		for fieldname in ("option_1", "option_2", "option_3"):
			...
			selected_options = [str(value).strip() for value in selected_options if str(value).strip()]
			if selected_options:
		...
		sql = f"""
				...
				order by REGEXP_REPLACE(p.name, '[0-9]', ''),cast(SUBSTRING_INDEX(p.name, '-', -1) as float)
			"""
		...
		if self.transaction_type == "Purchase Order":
			...
			self.set("products",[row for row in self.products if flt(row.quantity) != 0 or flt(row.cost) != 0])
