<template>
    ...
    <div class="cursor-pointer bg-green-600 text-white px-2 py-0  hover:bg-green-700">
        <div style="margin-bottom: 0px!important;" class="flex justify-between mb-2 text-lg">
            <div style="font-size: 22px;">{{$t('Grand Total')}}</div>
        ...
        <div class="flex justify-between mb-2 text-lg" v-if="data.changed_amount > 0">
            <div style="font-size: 22px;">{{ $t('Change Amount') }}({{ gv.setting.pos_setting.main_currency_name }}):</div>
            <div style="margin: 0px; padding: 0px; font-size: 26px; font-weight: bold;">
                <CurrencyFormat :value="data.changed_amount" />
            </div>
        </div>
        <div class="flex justify-between mb-2 text-lg" v-if="data.changed_amount > 0">
            <div style="font-size: 22px;">{{ $t('Change Amount') }}({{ gv.setting.pos_setting.second_currency_name }}):</div>
            <div style="margin: 0px; padding: 0px; font-size: 26px; font-weight: bold;">
        ...
        <div class="flex justify-between">
            <div style="font-size: 22px;">{{$t('Total Qty')}} : <span>{{ data.total_quantity }}</span></div>
            <div style="margin-bottom: 0px!important; align-items: center;" class="flex justify-between mb-2 text-lg">
                <ComExchangeRate />
                 <div style="margin: 0px; padding: 0px; font-size: 26px; font-weight: bold;">
                    <CurrencyFormat :value="(data.grand_total || 0) * (data.exchange_rate || 1)" :currency="gv.setting.pos_setting.second_currency_name" />
                </div>
            </div>
        </div>
    </div>
