...
class GenerateProducts(Document):
	...
	def on_submit(self):
		...
		frappe.enqueue("epos_restaurant_2023.inventory.doctype.generate_products.generate_products.bulk_insert_products",enqueue_after_commit=True,name=self.name,queue="long",enqueue_after_commit=True,job_name=f"Generate Products {self.name}")
