...
def add_sales_to_sync_queue(name,doctype,posting_date):
    ...
    for sale in sales:
        neted_sale = {"sale": sale, "products": []}
        neted_sale["products"] = [a for a in sale_products if a.get("parent") == sale.get("name")]
        doc = frappe.new_doc("Quickbooks Sync Queues")
        doc.action = "Add"
        doc.status = "Pending"
        doc.action_type = "Sale"
        doc.reference_name = sale
    ...
    for payment in sale_payments:
        ...
        doc.reference_name = payment
        doc.reference_doctype = "Sale Payment"
        doc.payload ="{\"posting_date\": \"" + str(posting_date) + "\",\"data\": " + str(json.dumps(payment)) + "}"
