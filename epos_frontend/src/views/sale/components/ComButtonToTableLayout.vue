<template>
  <template v-if="gv.setting.table_groups && gv.setting.table_groups.length > 0">
    ...
    <template v-else>
      <v-btn v-if="!mobile" stacked variant="elevated" size="small" class="m-0-1 grow"
        :prepend-icon="'mdi-keyboard-return'" @click="onToTableLayout()" style="font-family: Khmer OS Siemreap;">
        {{ $t('Back') }}
      </v-btn>
  ...
  <template v-else>
    ...
    <template v-else>
      <v-btn v-if="!mobile" stacked variant="elevated" size="small" class="m-0-1 grow"
        :prepend-icon="'mdi-keyboard-return'" @click="onToHomePage()" style="font-family: Khmer OS Siemreap;">
        {{ $t('Back') }}
      </v-btn>
      <v-btn v-else variant="tonal" size="small" :height="mobile ? '35px' : undefined" class="m-0-1 grow"
        @click="onToHomePage()">
